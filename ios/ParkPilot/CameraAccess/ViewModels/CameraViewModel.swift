...
final class CameraViewModel {
  /// Capture-only handoff to the API consumed by Hermes/OpenClaw.
  let captureBridge = CaptureBridge()
  private var shouldExposeNextPhoto = false
  // MARK: - Device
  ...
  /// Captures one DAT frame and uploads the resulting JPEG plus any recorded
  /// instruction. Upload starts only when photoDataPublisher delivers the photo.
  func exposeNextPhoto() {
    guard !captureBridge.isUploading else { return }
    shouldExposeNextPhoto = true
    if !capturePhoto() {
      shouldExposeNextPhoto = false
    }
  }
  ...
  private func handlePhotoData(_ data: PhotoData) {
    isCapturingPhoto = false
    if shouldExposeNextPhoto {
      shouldExposeNextPhoto = false
      Task { await captureBridge.upload(photoData: data.data) }
    }
