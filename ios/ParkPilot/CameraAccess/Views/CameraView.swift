...
struct CameraView: View {
  ...
  /// Minimal capture bridge controls. No parking logic runs in this app: the
  /// photo, optional audio/text instruction, and capture metadata are exposed to
  /// the configured Hermes/OpenClaw-facing endpoint.
  private var captureBridgePanel: some View {
    VStack(alignment: .leading, spacing: 10) {
      TextField(
        "Optional instruction for the agent",
        text: Binding(
          get: { viewModel.captureBridge.instruction },
          set: { viewModel.captureBridge.instruction = $0 }
        )
      )
      ...
      HStack(spacing: 10) {
        Button {
          viewModel.captureBridge.toggleInstructionRecording()
        } label: {
          Label(
            viewModel.captureBridge.isRecordingInstruction ? "Stop audio" : "Add audio",
            systemImage: viewModel.captureBridge.isRecordingInstruction ? "stop.fill" : "mic.fill"
          )
      ...
      Text(viewModel.captureBridge.status)
        .font(.caption)
